...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/08 18:55:09 by jheras-f          #+#    #+#             */
/*   Updated: 2020/09/08 18:55:25 by jheras-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

void		ft_separate_redir_r_aux(t_command *cmd, t_counters *grp, char *arg)
{
	grp->ret = 1;
	if (grp->len != 0)
	{
...
int			ft_separate_redir_r(t_command *cmd, char *arg)
{
	...
	while (grp.i < ft_strlen(arg))
	{
		...
		if (arg[grp.i] == '<')
			ft_separate_redir_r_aux(cmd, &grp, arg);
		grp.len++;
		grp.i++;
	}
	cmd->aux[cmd->cnt++] = ft_strsub(arg, grp.start, grp.len);
...
static void	ft_is_redir_r_aux(t_command *cmd, t_counters *grp, char *arg)
{
	if (arg[grp->i] == '<')
	{
		cmd->flag_redir_r++;
		if (ft_strlen(arg) > 1)
			grp->flag = ft_separate_redir_r(cmd, arg);
		else
			cmd->cnt_redir_r++;
	}
...
int			ft_is_redir_r(t_command *cmd, char *arg)
{
	...
	if (!arg || ft_strlen(arg) <= 0)
		return (0);
	while (grp.i < ft_strlen(arg) && !grp.flag)
	{
		if (arg[grp.i] == '"' || arg[grp.i] == '\'')
			ft_nq(arg, &grp.i, &grp.len);
		ft_is_redir_r_aux(cmd, &grp, arg);
	}
	if (!grp.flag)
...
void		ft_look_redir_r(t_shell *sh)
{
	...
	while (j <= sh->scolon_count)
	{
		...
		sh->cmd[j].cnt = 0;
		sh->cmd[j].cnt_redir_r = 0;
		while (i < sh->cmd[j].words)
			ft_is_redir_r(&sh->cmd[j], sh->cmd[j].args[i++]);
