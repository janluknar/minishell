...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/08 17:11:49 by jheras-f          #+#    #+#             */
/*   Updated: 2020/09/08 17:12:46 by jheras-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_join(t_file *file)
{
	...
	if (sz < file->len + BUFFER_SIZE)
		sz += BUFFER_SIZE;
	tmpstr = malloc((sz + 1) * sizeof(char));
	if (!tmpstr)
		return (1);
...
static int		ft_return(t_file **pfile, t_file *file, char **line)
{
	...
	if (file->rsz < 0)
		return (-1);
	*line = ft_strsub(file->strbuf, 0, file->endl - file->strbuf);
